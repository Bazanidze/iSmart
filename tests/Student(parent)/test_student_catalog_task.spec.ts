...
test.describe("Tests Student(parent) Catalog Tasks", async () => {
  test("Выбор фильтров", async ({ page }) => {
    const classNum = numberClass[0];
    const item = nameItem.logic;
    await allure.feature("Тесты: Ученик (родитель)");
    ...
    await catalogPage.usersCabinet.clickParentMyChilds();
    await catalogPage.parentMyChild.clickButtonLogCabinetStudent();
    await catalogPage.studentTasks.waitParentCabinetButton();
  ...
  test("Открытие раннера из предмета", async ({ page }) => {
    ...
    const buttonClass = await catalogPage.catalogPageEl.chooseClassPageItem(numberClass[5]);
