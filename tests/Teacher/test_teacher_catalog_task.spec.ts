...
test.describe("Tests Teacher Catalog Tasks", async () => {
  ...
  test("Открытие раннера из предмета", async ({ page }) => {
    await allure.feature("Тесты: Учитель");
    await allure.story("Каталог заданий");
    const catalogPage = new UsersCabinetsPage(page);
    await tokenEnv(page, defaultUsers.teacher[1], defaultUsers.teacher[2]);
    await catalogPage.open("");
    const errors: { url: string; status: number; payload?: any }[] = [];
    page.on("response", async (response) => {
      const status = response.status();
    ...
    await catalogPage.header.clickСatalogTasks();
    await catalogPage.catalogPageEl.clickSchoolGeometry();
    const buttonClass = await catalogPage.catalogPageEl.chooseClassPageItem(numberClass[8]);
    await catalogPage.catalogPageEl.clickFirstSubtopicsStudent();
